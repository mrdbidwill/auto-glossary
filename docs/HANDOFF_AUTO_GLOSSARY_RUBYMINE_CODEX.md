...
- Public-only gating via `ApplicationController#adsense_authenticated?` (checks `user_signed_in?`, `current_user`, `logged_in?`, `authenticated?`, or `session[:user_id]`). Override if needed.
